...
class FrontendApp extends HTMLElement {
  ...
  setBlogPostIsSaving(userId: string, isSaving: boolean) {
  ...
  removeUnsavedBlogPostFromUsers(userId: string) {
    this.store.users = this.store.users.map((user) => {
      if (user.id === userId) {
        return {
          ...user,
          blogPosts: user.blogPosts.filter(
            (blogPost) => blogPost.id !== "NOT_SET"
          ),
        };
      }
  ...
  setBlogPostInputText(userId: string, blogPostInputText: string) {
      ...
      <div class="main-container">
        <div class="frontend-create-user-container">
          <frontend-create-user
            ...
            @username-input-text-changed=${(e: UsernameInputTextChangedEvent) =>
              (this.store.usernameInputText = e.detail.usernameInputText)}
          ></frontend-create-user>
        </div>
        ...
        <div class="frontend-users-container">
          <frontend-users
            ...
            .users=${state.users}
            @blog-post-input-text-changed=${(
              e: BlogPostInputTextChangedEvent
            ) =>
              this.setBlogPostInputText(
                e.detail.userId,
                e.detail.blogPostInputText
              )}
            @create-blog-post=${(e: CreateBlogPostEvent) =>
              this.createBlogPostAndUpdateUsers(e.detail.userId)}
          ></frontend-users>
        </div>
      </div>
    `;
...
async function createBlogPost(userId: string, title: string): Promise<boolean> {
  const result = await mutation(
    gql`
      mutation ($userId: String!, $title: String!, $publishedAt: Date!) {
        createBlogPost(
          input: {
